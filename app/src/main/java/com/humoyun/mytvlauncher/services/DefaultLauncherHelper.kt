...
class DefaultLauncherHelper(
    ...
    fun isDefaultLauncher(): Boolean =
        isCompatible && roleManager?.isRoleHeld(RoleManagerCompat.ROLE_HOME) == true

    @SuppressLint("NewApi")
    fun canRequestDefaultLauncher(): Boolean =
        isCompatible && roleManager?.isRoleAvailable(RoleManagerCompat.ROLE_HOME) == true

    @SuppressLint("NewApi")
    fun requestDefaultLauncherIntent(): Intent? =
        if (isCompatible) roleManager?.createRequestRoleIntent(RoleManagerCompat.ROLE_HOME) else null
}